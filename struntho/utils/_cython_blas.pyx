...
cdef void _gemm(int order, int ta, int tb, int m, int n,
    ...
    # assign normally
    ta_ = Trans if ta == 1 else NoTrans
    tb_ = Trans if tb == 1 else NoTrans
    if order == 0:
        if floating is float:
...
cpdef _gemm_memview(int ta, int tb, floating alpha,
                    floating[:, :] A, floating[:, :] B, floating beta,
                    floating[:, :] C):
    cdef:
        int m = A.shape[0] if ta == 0 else A.shape[1]
        int n = B.shape[1] if tb == 0 else B.shape[0]
        int k = A.shape[1] if ta == 0 else A.shape[0]
        int lda, ldb, ldc
        int order = 1 if A.strides[0] == A.itemsize else 0

    if order == RowMajor:
        lda = k if ta == 0 else m
        ldb = n if tb == 0 else k
        ldc = n
    else:
        lda = m if ta == 0 else k
        ldb = k if tb == 0 else n
        ldc = m
